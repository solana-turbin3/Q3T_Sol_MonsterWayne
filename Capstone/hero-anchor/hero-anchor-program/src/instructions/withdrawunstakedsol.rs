    ...
    pub fn withdraw_unstaked_sol(&mut self) -> Result<()> {
        ...
        match stake_state {
            ...
            StakeStateV2::Stake(meta, stake, _) => {
                // Check if the stake is fully deactivated
                let clock = Clock::get()?;
                if stake.delegation.deactivation_epoch == std::u64::MAX 
                   || clock.epoch <= stake.delegation.deactivation_epoch {
                    return Err(ErrorCode::StakeAccountNotDeactivated.into());
                }
                // Additional check: ensure all rewards have been claimed
                if meta.rent_exempt_reserve != stake_account_info.lamports() {
        ...
        // Calculate the amount to withdraw (total balance minus rent-exempt reserve)
        let rent_exempt_reserve = Rent::get()?.minimum_balance(stake::state::StakeStateV2::size_of());
        let withdraw_amount = stake_account_info.lamports().saturating_sub(rent_exempt_reserve);
