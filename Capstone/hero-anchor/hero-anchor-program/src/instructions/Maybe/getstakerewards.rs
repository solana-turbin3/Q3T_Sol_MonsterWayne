...
use actix_web::{post, App, HttpServer, Responder, HttpResponse, Error};
...
// Define an Actix Web handler
#[post("/get-stake-rewards")]
async fn get_stake_rewards_endpoint() -> Result<HttpResponse, Error> {
    // Initialize your program context here if necessary
    // For example, set up accounts, context, etc.
    
    // Create an instance of GetStakeRewards and call the function
    let mut get_stake_rewards = GetStakeRewards {
        // Initialize necessary fields
    };
    
    match get_stake_rewards.getstakerewards() {
        Ok(rewards) => Ok(HttpResponse::Ok().body(format!("Rewards: {}", rewards))),
...
async fn main() -> std::io::Result<()> {
    HttpServer::new(|| {
        App::new()
            .service(get_stake_rewards_endpoint)
    })
    .bind("0.0.0.0:8080")?
    .run()
    .await
