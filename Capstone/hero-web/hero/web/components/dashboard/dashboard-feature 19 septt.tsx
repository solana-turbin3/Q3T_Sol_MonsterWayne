...
const PROGRAM_ID = new PublicKey('ARNRpPKnDd2CHbAJMDFeog5DKChgPtqCxp2CDJYMZXJh');
...
export default function DashboardFeature() {
  ...
  const fetchCreatorData = async () => {
    ...
    try {
      ...
      try {
        ...
        setCreatorPDA(creatorPDA.toBase58());
        console.log('Creator PDA Data:', account);
      } catch (error) {
  ...
  const fetchUserData = async () => {
    ...
    try {
      const program = getProgram();
      const creatorPublicKey = new PublicKey(creatorPubkey);
      ...
      console.log('User Vault PDA Data:', userAccount);

      // Fetch the stake account data after fetching user data
      fetchStakeAccountData(userAccount);
    } catch (error) {
          ...
          <div className="flex flex-col space-y-4">
            ...
            {creatorData && (
              <div className="mt-4">
                ...
                <div className="bg-gray-100 p-3 rounded-md">
                  <p><strong>Name:</strong> {creatorData.name}</p>
                  <p><strong>Total Staked:</strong> {creatorData.totalStaked?.toString()} lamports</p>
                  <p><strong>Total Rewards:</strong> {creatorData.totalRewards?.toString()} lamports</p>
                </div>
              </div>
            )}
            ...
            {userData && (
              <div className="mt-4">
                ...
                <div className="bg-gray-100 p-3 rounded-md">
                  <p><strong>Creator:</strong> {userData.creator?.toBase58()}</p>
                  <p><strong>Stake Account:</strong> {userData.stakeAccount?.toBase58()}</p>
                  <p><strong>Staked Amount:</strong> {userData.stakedAmount?.toString()} lamports</p>
                  <p><strong>Rewards:</strong> {userData.rewards?.toString()} lamports</p>
                </div>
              </div>
            )}
          </div>
