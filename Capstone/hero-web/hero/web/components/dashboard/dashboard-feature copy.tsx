...
import { PublicKey, Keypair, StakeProgram, LAMPORTS_PER_SOL, Authorized, Transaction, sendAndConfirmTransaction, Lockup } from '@solana/web3.js';
...
const PROGRAM_ID = new PublicKey('5s6dWjXhUnv7EjVfAput2x8WAxB5vBsSQpKZbnDM4ZVC');
...
export default function DashboardFeature() {
  ...
  const initializeUser = async () => {
    if (!publicKey || !initAmount) {
      console.log('No public key available or amount not specified');
      return;
    }
    try {
      const program = getProgram();
      ...
      const creatorPublicKey = new PublicKey("tVZyEqNfxXvFz5TZaRvggfxAnqjHksZdCp9BRQnXs35"); // Replace with actual creator public key
      ...
      const [creatorPDA] = await PublicKey.findProgramAddress(
        [Buffer.from('creator_pda'), creatorPublicKey.toBuffer()],
        program.programId
      );

      console.log('Calling inituser and deposit methods');
      const tx = await program.methods.inituser(amount, creatorPublicKey)
        .accounts({
          user: publicKey,
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      // If the account doesn't exist, setUserData to null
      setUserData(null);
      setUserPDA(null);
    }
  };
          ...
          {!userData ? (
            <div>
              ...
              <button
                ...
                disabled={!initAmount || parseFloat(initAmount) <= 0}
